...
if ($AndMonarch) {
    ...
    Write-Host '  MONARCH PREFLIGHT + AUTO-LAUNCH' -ForegroundColor Cyan
    Write-Host '=================================================================================' -ForegroundColor Cyan
...
# --- 0. Check for administrator ---
$isAdmin = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] 'Administrator')
if (-not $isAdmin) {
    Write-Host 'preflight FAILED: This script and Monarch require administrator privileges' -ForegroundColor Red
    Write-Host '  -> fix: run PowerShell from an account with Domain Admin rights' -ForegroundColor Yellow
    return
}

# --- 1. Remove cached module ---
Write-Host 'preflight: forcing unload of Monarch...' -ForegroundColor DarkGray
